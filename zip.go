...
func extractZip(src, destDir, password string, verbose bool) (int, error) {
	r, err := zip.OpenReader(src)
	if err != nil {
		return 0, fmt.Errorf("ZIP arşivi açılamadı: %w", err)
	}
	defer r.Close()
	...
	for _, f := range r.File {
		...
		count++
		if verbose {
			fmt.Printf("  -> [OK] %s (%s)\n", f.Name, formatSize(int64(f.UncompressedSize64)))
		}
	}
